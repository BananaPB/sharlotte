...
- `ingredient_categories` and `allergens` — small lookup tables (`code` is the stable identity to join/compare on, `label_fr` is display-only). `IngredientCategorySeeder` currently seeds 7 of the source spreadsheet's 20 real-world categories; the rest are pending more data from the project owner — current state, not a defect.
