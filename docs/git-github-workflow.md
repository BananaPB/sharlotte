...
- **`docs/current-state`** — a branch you (or a Claude session) created for a specific chunk of work (recording the repo's current state in the docs). It's _supposed_ to be temporary: it exists to be merged into `main` and then deleted.
...
A repo with active work in it _always_ has more than one branch. One branch (`main`) would mean either nothing is being worked on, or people are committing straight to it unreviewed — which is the thing branches exist to avoid.

## 3. `git push` vs. Pull Request — two different things

This is the part that's easy to conflate, because both involve "sending work to GitHub." They are not the same operation, and they're not even the same _kind_ of thing.
...
Concretely: you `git push` your branch so GitHub _has_ your commits. You then open a **Pull Request** to say "please merge `docs/current-state` into `main`." The push is a delivery; the PR is a proposal with a review process attached.
...
3. **Branch protection rules** — settings on `main`, configured in _Settings → Branches_ on GitHub, that say "don't allow a merge unless conditions 1 and 2 are satisfied." This is what produces the red X's and the greyed-out "Merge pull request" button in your screenshot — it's not an error, it's the rule working exactly as configured: _"I will not let you merge until CI is green and someone has approved."_
...
## 5. The GitHub Actions running in _this_ repo
...
## 9. What's actually broken in _this_ repo, right now

Three independent problems are stacked in your screenshot. None of them is "the initial commit failed" in the sense of git losing anything — nothing is lost, your commits are all there. What's failing is the _gate_ GitHub puts in front of merging. Here's each one, and how I confirmed it, so you can check my work and try the fix yourself.
...
This means: as currently configured, _no PR you open can ever be merged_, not because anything is wrong with your code, but because the rule assumes a second person exists.

**How to verify it yourself**: GitHub → your repo → _Settings → Branches_ (or _Rules → Rulesets_) → the rule protecting `main` → look at "Require a pull request before merging" → "Require approvals."
...
This tells `npm` itself: _"refuse to run unless you are pnpm."_ But nothing else was updated to match — [.github/workflows/tests.yml](../.github/workflows/tests.yml) never sets up pnpm, and `composer.json`'s `setup` script still calls plain `npm install` / `npm run build`. The pin was added without switching what actually runs, so `npm` refuses to install anything, immediately, before the frontend build even starts.
...
### 9.4 `npm run check` / `npm run build` can't run locally from Git Bash — confirmed, not a config problem

This is the "unrelated Windows-only problem" mentioned in §9.3 above, tracked down properly rather than left as a shrug. `npm run check` (and `build`) run `vp`, a compiled tool from the `vite-plus` package (`@voidzero-dev/vite-plus-*`, a native Rust/N-API binary, not plain JS). From a Git Bash session it fails one of two ways depending on npm's `script-shell` setting:

- default (`cmd.exe`): `'"node"' n'est pas reconnu en tant que commande interne...` — a quoting bug in how npm builds the `cmd.exe` command line.
- `script-shell` set to `bash` or `powershell`: past that, but then `vp` itself fails with `error: Cannot find binary path for command 'node'` — its own internal lookup can't parse the POSIX-style `PATH` (`/c/Program Files/nodejs:...`) that Git Bash (MSYS) exports, and it needs Windows-style (`C:\Program Files\nodejs;...`).

Both were reproduced directly, and several standard workarounds were tried and ruled out: converting `PATH` to Windows format with `cygpath -wp` before invoking `npm` (breaks bash's own ability to resolve `npm`/`bash`, since bash needs POSIX-style `PATH` for its own lookups while `vp` needs Windows-style for its own — one variable can't satisfy both in the same process tree); running `npm` through a real child `powershell.exe` process with a converted `PATH` (still hits the cmd.exe quoting bug — proving that failure isn't actually about Git Bash at all, since it reproduces even starting from a clean native process); checking for the classic trailing-backslash-in-`PATH` cause of that quoting bug (not present here).

**The shape of the fix**: there isn't one that lives in this repo. The bug is either in npm's Windows `cmd.exe` script runner or in `vite-plus`'s compiled binary — both third-party, closed to a one-line fix, and not worth the hours it'd take to chase inside a compiled Rust addon on a project with no users yet. **What actually works, confirmed**: open a genuinely fresh, top-level PowerShell or cmd window — not one spawned from inside Git Bash — and run `npm run check:fix` there. A fresh window loads a correct native Windows `PATH` from the start and never crosses the Git Bash/MSYS boundary that causes both failures above. `/cto`'s wrap-up mode now reminds you to do this before every push.
...
When the time comes, the review-approval rule from §9.1 doesn't get _disabled_ — you make yourself exempt from it while leaving it on for everyone else. That's a different, better fix than the one in §9.1's "shape of the fix," and worth doing this way from the start:

- **Classic branch protection**: uncheck **"Include administrators."**
- **Newer Rulesets** (_Settings → Rules → Rulesets_): add yourself to the ruleset's **bypass list**.
...
1. **Gate first-time contributors' CI runs.** _Settings → Actions → General → "Fork pull request workflows"_ → **"Require approval for first-time contributors"** (GitHub's own default). A stranger's first PR then needs you to click "Approve and run workflows" before any CI executes on it — after that first reviewed PR, later ones from the same person run automatically. This stops someone from using your CI minutes, or probing your CI environment, via a PR you haven't looked at yet.
2. **Never switch [ai-pr-review.yml](../.github/workflows/ai-pr-review.yml) from `pull_request` to `pull_request_target`.** The current trigger means a PR from a fork runs with **no access to your repo secrets**, including `CLAUDE_CODE_OAUTH_TOKEN` — this is GitHub's default protection, and it's already correct. `pull_request_target` runs the workflow with your secrets available _and_ against the base repo's permissions while still checking out the contributor's code — that combination is the single most common way real open-source projects get their secrets exfiltrated, via a crafted first PR. If the AI review simply doesn't run on external PRs, that's the safe failure mode, not a bug to "fix" by switching the trigger.
